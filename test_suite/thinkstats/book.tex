...
Giant\textregisteredpumpkin that weighs 591 pounds.  The mean of
...
%
The term \xsubi-\mymuis called the ``deviation from the mean,'' so
...
variance with \n\minus1 in the denominator, rather than \n.  This
...
\[ \mu = \sum_i p_i x_i \]
%
where the \xsubiare the unique values in the PMF and \psubi=PMF(\xsubi).
...
Plot this value as a function of \xfor first babies and others.
...
The CDF is a function of \x, where \xis any value that might appear
...
If \xis less than the smallest value in the sample, CDF(\x) is 0.
If \xis greater than the largest value, CDF(\x) is 1.
...
One way is to plot the complementary CDF, 1\minusCDF(\x), on a
log-\yscale.  For data from an exponential distribution, the result
...
So on a log-\yscale the CCDF is a straight line
...
on a log-\yscale.  It is not exactly straight, which suggests that
...
distribution with mean 32.6.  Plot the CCDF on a log-\yscale and
compare it to Figure\ref{interarrival_ccdf}.
\index{pyplot}

Hint: You can use the function {\tt pyplot.yscale} to plot the \yaxis
on a log scale.
...
transform the axes; to plot a CCDF on a log-\yscale:
%
\begin{verbatim}
...
times and the CCDF on a log-\yscale.  Does it look like
...
%
The parameters \x \sub{m} and \myalphadetermine the location and shape of
...
\Eqn{ log y \myapprox \minus\myalpha(log \x\minuslog x\sub{m}) }

So if you plot log \yversus log \x, it should look like a straight
line with slope \minus\myalphaand intercept
\myalphalog \x\sub{m}.
...
\myalphaand \x \sub{m} as parameters and uses {\tt
...
%
The parameters \mymuand \mysigmadetermine the mean and standard
...
dataset and \mis a multiplier.  Then select every \m th element,
...
of log \w, where \wis weight in kilograms, along with a normal
model.
\index{respondent}
\index{model}
...
transform.  Since the distribution of log \wfits a normal distribution, we
conclude that \wfits a lognormal distribution.
\index{normal distribution}
\index{distribution!normal}
\index{Gaussian distribution}
\index{distribution!Gaussian}
...
generates normal probability plots for \wand log \w.  You can
...
\item Plot the CDF on linear and log-\xscales so you can get a sense
...
not 1, so 1\minus\pcan be 1 but not 0, which is good, because log 0 is
undefined.
\index{random module}
...
\Erepresents an event, then \Prob(\E) represents the probability that
\Ewill occur.  A situation where \Emight or might not happen is
called a {\bf trial}.
\index{success}
\index{failure}
\index{dice}
...
If we have a finite sample of \ntrials and we observe \ssuccesses,
...
where \Prob(\A \AND \B) is the probability that events \Aand \Bboth
occur.  This formula is easy to remember; the only problem is that it
is {\em not always true}.  This formula only applies if \Aand \Bare 
{\bf independent}, which means that if I know \Aoccurred, that
doesn't change the probability of \B, and vice versa.
\index{independent}
\index{event!independent}

For example, if \Ais tossing a coin and getting heads, and \Bis 
rolling a die and getting 1, \Aand \Bare independent, because
the coin toss doesn't tell me anything about the die roll.
\index{dice}

But if I roll two dice, and \Ais getting at least one six, and
\Bis getting two sixes, \Aand \Bare not independent, because
if I know that \Aoccurred, the probability of \Bis higher, and
if I know \Boccurred, the probability of \Ais 1.
\index{conditional probability}
\index{probability!conditional}

When \Aand \Bare not independent, it is often useful to compute
the conditional probability, \Prob(\A|\B), which is the probability of
\Agiven that we know \Boccurred:
%
...
These relationships hold whether \Aand \Bare independent or not.
...
Henri \Poincarewas a French mathematician who taught at the Sorbonne
around 1900.  The following anecdote about him is probably fabricated,
but it makes an interesting probability problem.
\index{bread police}

Supposedly \Poincaresuspected that his local bakery was selling
...
For the next year, \Poincarecontinued the practice of weighing his
...
loaves, but deliberately giving \Poincarethe heavier ones.
...
exclusive.  In general the probability of \Aor \Bor both is:
...
getting at least one tails is 1/2+1/2\minus1/4.  I have to subtract
...
\begin{exercise}
What is the general formula for the probability of \Aor \Bbut not both?
...
chance of getting \kksixes, for all values of \kkfrom 0 to 100.  The
answer is the {\bf binomial distribution}, which has this PMF:
%
\[ \PMF(k) = \binom{n}{k} p^k (1-p)^{n-k}\]
%
where \nis the number of trials, \pis the probability of success,
and \kkis the number of successes.
...
%
In words, this equation says that the probability of \HHafter you
have seen \Eis the product of \Prob(\HH), which is the probability of
\HHbefore you saw the evidence, and the ratio of \Prob(\E|\HH), the
probability of seeing the evidence assuming that \HHis true, and
...
\Nis the hypothesis that the subject of the test does not use drugs:
...
a value generated by the random process \Xis less than or equal
to \x.
...
\kkvalues from an exponential distribution with the same \mylambda.
...
%
Evaluating a PDF for a particular value of \xis usually not useful.
...
For example, if \xis a random variable whose PDF is PDF\sub{X}, we
can compute the probability that a value from \Xfalls between 
\minus0.5 and 0.5:
%
...
distribution with parameter \mylambdafalls between 1 and 20?  Express
...
Suppose we have two random variables, \Xand \Y, 
...
Given any RandomVariables, \Xand \Y, we can create a Sum
object that represents \Z.  Then we can use a sample from \Zto
...
the first term is \xand the sum has to be less than \z, then the
second term has to be less than \z\minus\x.

\item To get the probability that \Yis less than \z\minus\x, we
evaluate CDF\sub{Y}.
%
...
%
The integrand is ``the probability that \Zis less than or equal
...
\Eqn{ PDF\sub{Z} = PDF\sub{Y} \mystarPDF\sub{X} }
...
As an example, suppose \Xand \Yare random variables with an
exponential distribution with parameter \mylambda.  
The distribution of \Z=\X+\Yis:
%
...
\begin{exercise}
If \Xhas an exponential distribution with parameter
\mylambda, and \Yhas an Erlang distribution with parameters
\kkand \mylambda, what is the distribution of the sum \Z=\X+\Y?
\index{exponential distribution}
\index{distribution!exponential}
...
normal with parameters \mymuand \mysigma, you can write

\Eqn{ \X\mysim\mynormal (\mymu, \mysigma\super{2}) }

where the symbol \mysimmeans ``is distributed'' and the script letter
\mynormalstands for ``normal.''
...
A linear transformation of \Xis something like \X'=\mya \X+\myb, where
\myaand \mybare real numbers.\index{linear transformation}
...
%The CDF of the chi-squared distribution with \kkdegrees of freedom is
%
%\[ \chi^2(k, x) = P(k/2, x/2) \]
%
%where \pis the regularized gamma function (see
...
standard deviation \mymuand \mysigma, the distribution of the sum is
...
distribution with finite mean \mymuand variance \sigmasq, what is
...
generated samples with sizes \nand \mand computed the difference
in mean.
\index{resampling}
...
effect as big as \mydeltaabout 17\% of the time, even if the actual
...
a p-value less than \myalpha.  A common choice for \myalphais 5\%.
...
\Prob(\E|\HH\sub{0}), where \Eis an effect as big as or bigger
...
\item[Bayesian:] What we really want to know is \Prob(\HH\sub{A}|\E), 
  ...
  \Eunder any hypothesis.  Since the effect is either real or it's not,
...
chance.  If we use the observed value of \mydeltato formulate the
hypothesis, \Prob(\HH\sub{A}|\E) is likely to be high even when
\HH\sub{A}is false.

We can address this problem with {\bf cross-validation}, which uses
one dataset to compute \mydeltaand a {\em different} dataset to
...
divide by \n.  If the distribution has mean \mymuand variance
...
one of the properties of the normal distribution: if \Xis
...
\item[likelihood ratio:] The ratio of \Prob(\E|\A) to \Prob(\E|\B) 
  for two hypotheses \Aand \B, which is a way to report
...
until now we have used the symbol \mymufor both the sample mean and
the mean parameter, but now to distinguish them I will use \myxbarfor 
the sample mean.  In this example, \myxbaris 0.155, so it would
...
Using the sample mean to estimate \mymuis so obvious that it is hard
...
%
Where \mis the number of times you play the estimation game (not to
...
\mymu=0 and \mysigma=1.  Use the sample mean to estimate \mymuand
...
\sigmasqand compute the error \Ssq\minus\mysigma\super{2}.
...
So if you want to maximize your chance of guessing \mylambdaexactly,
$\lamhat$ is the way to go.
\index{MLE}
\index{maximum likelihood estimator}

But we know that \myxbaris not robust in the presence of outliers, so
...
and over, your interval will contain \mylambda90\% of the time.
...
The confidence interval for the \mylambdaparameter of an exponential
distribution is
%
...
where \nis the sample size, $\lamhat$ is the mean-based estimator
      ...
      CDF of a chi-squared distribution with \kkdegrees of freedom,
      evaluated at \x(see
...
distribution, and I chose \mylambdafrom a uniform distribution
between 0.5 and 1.5.  Here's a sample, which I'll call \X:

\{2.675, 0.198, 1.152, 0.787, 2.717, 4.269\}

Based on this sample, what value of \mylambdado you think I chose?
...
actual value of \mylambdafalls in the \iith bin.
Since \mylambdawas drawn from a uniform distribution, the prior
probability, \Prob(\HH\sub{i}), is the same for all \ii.

\item For each hypothesis, we compute the likelihood, \Prob(\X|\HH\sub{i}),
which is the chance of drawing the sample \Xgiven \HH\sub{i}.
%
...
where \fis the normalization factor
%
...
value 0.5 represents the hypothesis that \mylambdais 0.5.
...
\mylambdafor the sample \X=\{1.5, 2, 3, 4, 5, 12\}.  For
...
\item If we see train \iiit seems reasonable that we would guess
  some multiple of \iiso let's assume \mynhat=\mya \ii.  What value of
  \myaminimizes mean squared error?
\index{MSE}
\index{mean squared error}

\item Still assuming that \mynhat=\mya \iican you find a value of \
\myathat makes \mynhatan unbiased estimator?
\index{unbiased estimator}
\index{estimator!unbiased}
...
For a given estimate, \mynhat, the likelihood of seeing train \iiis
...
Suppose that there are, in fact, \Ntrains.  Each time we play
the estimation game, we see train \iiand guess \mya\ii, so the squared
error is (\mya\ii\minus\N)\super{2}.

If we play the game \Ntimes and see each train once, the mean
squared error is 
%
...
\[ a = \frac{3N}{2N+1} \]
%
At first glance, that doesn't seem very useful, because \Nappears on
the right-hand side, which suggests that we need to know \Nto choose
\mya, but if we knew \N, we wouldn't need an estimator in the first place.

However, for large values of \N, the optimal value for \myaconverges
...
%
And find the value of \myathat yields ME=0, which turns out to be
%
\[ a = \frac{2N}{N-1}\]
%
For large values of \N, \myaconverges to 2, so we could choose
\mynhat=2\ii.
...
same mean is \mynhat=2\ii\minus1.
...
If \Xis a series of values, \xsubi, we can convert to standard
...
\mysigma{\bf normalizes} the deviation, so the values of \Zare
...
If \Xis normally-distributed, so is \Z; but if \Xis skewed or has
...
to vary together.  If we have two series, \Xand \Y, their
...
where \mymu \sub{X} is the mean of \Xand \mymu \sub{Y} is the mean of \Y.
If \Xand \Yvary together, their deviations tend to have the same
sign.
...
units of \Xand \Y.  So the covariance of weight and height might be
...
can rewrite \myrhoby factoring out \mysigma \sub{X} and \mysigma \sub{Y}:
%
...
that \myrho\super{2}\myle1, so \minus1\myle\myrho\myle1.
\index{Cauchy-Schwarz inequality}

The magnitude of \myrhoindicates the strength of the correlation.  If
...
nonlinear relationship, \myrhounderstates the strength of the
dependence.
...
\myrholook like.  The second row shows perfect correlations with a
...
between \Xand \Ywith intercept \myinterand slope \myslope, we
...
If we get the parameters \myinterand \myslopewrong, the residuals
...
\item If the residuals are independent of \x, random, and normally
  ...
  \myinterand \myslope.\footnote{See Press et al., {\em Numerical Recipes in C},
...
For example, if you are using values of \Xto predict values of \Y,
...
\item Compute the sample means, \myxbarand \myybar, the variance
of \X, and the covariance of \Xand \Y.
...
\begin{exercise}
Write a function named {\tt LeastSquares} that takes \Xand \Yand
...
\myeps\sub{i}and \Yand returns \R\super{2}.  To test your functions, 
